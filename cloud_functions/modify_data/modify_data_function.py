...
def get_google_sheet_data():
    ...
    delete_query = "SELECT employee_id, employee_first_name, employee_last_name, " \
                   ...
                   f"employee_salary_currency, updated_timestamp FROM `{table_name}` "
    ...
    check_time = datetime(year=9999, month=12, day=31, hour=23, minute=59, second=59)

    if query_response.done():
        try:
            query_response.result()
            for row in list(query_response):
                if row.years_left_for_retirement > 0 and row.updated_timestamp != check_time:
...
def insert_data_to_postgres(employee_table_data):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            """, employee_table_data)
            connection.commit()
            logging.info(cursor.rowcount, "Records modified successfully from employee_details table")
    except Exception as e:
        logging.error("Error in updating data to postgres instance", str(e))
        return abort(500, json.dumps({"status": "Failed", "message": "Finished with error. Check Logs for more "
                                                                     "details"}))
    finally:
        if connection:
